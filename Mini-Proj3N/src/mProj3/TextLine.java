...
public class TextLine implements TextBlock {
 ...
 public boolean eqv(TextBlock cmp) {
	  ...
	  if (cmp == null) {// checks for null case
		  return false;
	  } else if(!(cmp instanceof TextLine)) {
		  return false;
	  } else{ 
		  TextLine cmpLine = ((TextLine) cmp);
		  return this.line.equals(cmpLine.line); // returns true if line is the same
	  }
 }
